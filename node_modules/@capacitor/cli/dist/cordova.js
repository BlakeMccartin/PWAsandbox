...
async function copyPluginsJS(config, cordovaPlugins, platform) {
    ...
    await Promise.all(cordovaPlugins.map(async (p) => {
        ...
        jsModules.map(async (jsModule) => {
            const filePath = path_1.join(webDir, 'plugins', p.id, jsModule.$.src);
